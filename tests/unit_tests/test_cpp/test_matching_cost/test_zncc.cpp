...
TEST_CASE_TEMPLATE("ZNCC", Matrix, P2d::Matrixf, P2d::MatrixD) {
  SUBCASE("ZNCC with two identical images gives 1") {
    ...
    auto zncc_opt1 = calculate_zncc_opt1(integral_left, integral_left_sq, integral_right,
                                         integral_right_sq, integral_cross, 0, 0, 2, 2, 3);
    ZnccOpt2Correlator<float> correlator;
    correlator.prepare_left_window(image);
    auto zncc_opt2 = correlator(image);
  ...
  SUBCASE("Null standard deviation gives 0") {
    SUBCASE("Left standard deviation is null") {
      ...
      auto zncc_opt1 = calculate_zncc_opt1(integral_left, integral_left_sq, integral_right,
                                           integral_right_sq, integral_cross, 0, 0, 2, 2, 3);
      ZnccOpt2Correlator<float> correlator;
      correlator.prepare_left_window(left_image);
      auto zncc_opt2 = correlator(right_image);
  ...
  SUBCASE("ZNCC with two anticorrelated images") {
    ...
    auto zncc_opt1 = calculate_zncc_opt1(integral_left, integral_left_sq, integral_right,
                                         integral_right_sq, integral_cross, 0, 0, 4, 4, 5);
    ZnccOpt2Correlator<float> correlator;
    correlator.prepare_left_window(left_image);
    auto zncc_opt2 = correlator(right_image);
