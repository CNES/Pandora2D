...
That means we are looking for the min (resp. max for zncc and mutual information measures).
For column disparities (resp. row disparities) we search the min or max in disp_row (resp. disp_col). 
